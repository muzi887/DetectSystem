...
| **气象 Tab** | 三监测站各 9 项读数 Mock 驱动 → 下拉切换 → 副标题 / AI 随站变化 |
...
| 延伸 | 气象 Tab 三站 9 项动态数据 | ✅ | [气象Tab动态数据学习笔记.md](./气象Tab动态数据学习笔记.md) |

**建议阅读顺序（新人）：** P1-3 → P1-1 → P1-2（无人机线）→ P1-4 → P1-5 → P1-6 → P1-7（GIS 线）→ [气象Tab动态数据学习笔记.md](./气象Tab动态数据学习笔记.md)（气象线）。
...
| `src/views/user/RelatedData.vue` | compare props、查值状态、动态 AI、气象 Tab 三站切换 |
| `src/stores/data.ts` | `weatherReadings` / `fetchWeatherReadings` / `getWeatherReadingByPointId` |
| `src/mock/db.json` / `deploy/api_mock/db.json` | `weatherReadings` 三站 9 项 Mock |
...
```text
气象 Tab → 监测站下拉（河间 / 雄县 / 栾城）
→ GET /weatherReadings → 9 张卡片（土壤 3 + 大气 6）
→ 副标题显示当前站名 → AI 按墒情 / 湿度 / 降雨生成
→ 若从无人机 Tab 切来，自动联动 fields.monitorPointId 对应站
```

**数据口径：** 每站各一套 9 项（非 9 项拆成每站 3 项）；雄县偏干、栾城偏湿与 `monitorPoints.soilMoisture` 一致。详见 [气象Tab动态数据学习笔记.md](./气象Tab动态数据学习笔记.md)。
...
| 延伸 | 气象 Tab 三站切换 + 动态 9 项 | ✅ | `GET /weatherReadings` + 站下拉 |
...
4. **气象**：[气象Tab动态数据学习笔记.md §10](./气象Tab动态数据学习笔记.md#10-本地验证)：切换三站 → 卡片 / 副标题 / AI 变化 → 无人机地块联动  
...
| [气象Tab动态数据学习笔记.md](./气象Tab动态数据学习笔记.md) | 气象 Tab Mock / Store / 三站 9 项 / 布局 |
| [地图组件纯props原则学习笔记.md](./地图组件纯props原则学习笔记.md) | 原则一 + Pinia/Store 补课 |

---

*文档版本：v1.1 · 补充气象 Tab 动态数据索引 · 2026-06-05*
