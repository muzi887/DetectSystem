...
> **当前产品（2026-08-24）**：23 类（小麦/玉米/番茄/水稻）、作物掩码、防治库已接线；磁盘 `pest-cls-best.pt` / `pest-cls-meta.json` 为 **23 类（验证集 94.05%）**。  
> **当前整理脚本**：`prepare_from_class_folders.py`（源图 `ml-bjj/data/<中文类名>/`）。本文 §零 的小麦 Kaggle + PlantVillage 流程对应 `prepare_from_wheat_plantvillage.py`，仅用于复现 8 类。  
> **下一步**：云上同步权重、竞赛材料、实地抽测；补图重训见 [`下一阶段任务与流程.md`](../../规划/下一阶段任务与流程.md)、[`玉米水稻补图-搜索词条.md`](../训练/玉米水稻补图-搜索词条.md)。
