...
> **定位**：在 **23 类 Serving 与磁盘权重均已就绪** 的前提下，把「难例 / 实地 / 可选更大骨干」叠在 **当前 23 类模型** 上。  
> 不再把 v3.1 定义成「永远 8 类、不改标签体系」。8 类是 2026-07 的首版；产品门禁与线上权重已是 23 类（验证集 94.05%）。  
> **当前不阻塞识病**：继续补图、实地抽测见 [`下一阶段任务与流程.md`](../../规划/下一阶段任务与流程.md)、[`玉米水稻补图-搜索词条.md`](../训练/玉米水稻补图-搜索词条.md)。  
...
| 维度 | v3 首版（已训） | 23 类扩展（B23 · ✅ 已就位） | v3.1（本文件 · 非阻塞） |
|------|-----------------|-----------------------------|-------------------------|
...
| **权重** | 历史 8 类 best | ✅ `pest-cls-best.pt` 23 类 / 94.05% | 备份为 `pest-cls-bjj-v3.1.pt`，验收后再替换 |
| **是否阻塞** | 阶段 A 已完成 | 不再阻塞识病 | 答辩前有余力再做 |
